...
if __name__ == "__main__":
    filepath = sys.argv[1]
    logname = (
        filepath.split("/")[-2] + "_" + filepath.split("/")[-1].split(".")[0]
    )
        ...
        if not learned:
            ...
            if isinstance(learned, ConditionalRule):
                if len(learned.choices) != choices:
                    logger.info(
                        f"Expected {choices} options for rule '{rule}', got {len(learned.options)}"
                    )
            else:
                if choices > 1:
                    logger.info(
                        f"Expected {choices} options for rule '{rule}', but it is not ConditionalRule"
                    )
