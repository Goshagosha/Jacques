...
def generate(length, redundancy):
    ...
    while len(pool) >= length:
        result = [].copy()
        to_remove = []
        chosen_indexes = random.sample(range(0, len(pool)), length)
        for i in chosen_indexes:
            c = pool[i]
            result.append(c)
            to_remove.append(c)
        if not mutex_check_pass(result):
            continue
    ...
    if len(pool) > 0:
        if not mutex_check_pass(pool):
            raise Exception("Mututally exclusive tokens left in the pool")
        pool.sort(key=sort_prefix)
        pool.sort(key=sort_postfix)
        no_definitions = True
        for definition in definitions:
